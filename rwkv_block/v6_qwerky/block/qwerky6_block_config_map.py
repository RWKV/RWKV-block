...
@dataclass
class Qwerky6BlockConfigMap(RWKV6BlockConfigMap):
    ...
    def new_block_config_map(self, **kwargs) -> 'Qwerky6BlockConfigMap':
        '''
        Returns a new config map with updated values
        '''

        new_dict = {}
        for key in Qwerky6BlockConfigMap.__dataclass_fields__:
            if key in self.__dict__:
                new_dict[key] = self.__dict__[key]
        new_dict.update(kwargs)

        return Qwerky6BlockConfigMap(**new_dict)

    @staticmethod
    def normalize(config_map: any) -> 'Qwerky6BlockConfigMap':
        '''
        Converts either maps, objs or Qwerky6BlockConfigMap
        '''
        if isinstance(config_map, Qwerky6BlockConfigMap):
            return config_map
        ...
        if dict_obj is not None:
            # Filter for only valeus in Qwerky6BlockConfigMap
            new_dict = {}
            for key, value in dict_obj.items():
                if key in Qwerky6BlockConfigMap.__dataclass_fields__:
                    new_dict[key] = value
            return Qwerky6BlockConfigMap(**new_dict)
