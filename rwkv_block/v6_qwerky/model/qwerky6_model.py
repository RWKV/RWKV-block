...
from .qwerky6_config_map import Qwerky6ConfigMap
from ..block.qwerky6_layer_block import Qwerky6LayerBlock

from transformers.models.qwen2.modeling_qwen2 import Qwen2RMSNorm, Qwen2DecoderLayer, Qwen2RotaryEmbedding

class Qwerky6Model(nn.Module):
    '''
    Qwerky6 Model architecture
    Simplified implementation

    Note: This EXCLUDES the head layer, keeping in line with the HF format convention
    '''

    def __init__(self, config: Union[Qwerky6ConfigMap, any, None] = None):
        # Initialize the base class
        super().__init__()

        # Normalize the config
        configMap:Qwerky6ConfigMap = Qwerky6ConfigMap.normalize(config)
        ...
        with torch.device(device):
            ...
            self.layers = nn.ModuleList([
                ...
                *[Qwen2DecoderLayer(config.hybrid_layer_config(), offset).bfloat16() for offset in range(config.num_prefix_hybrid_layers)],
                # Qwerky layers
                *[Qwerky6LayerBlock(config.new_block_config_map(layer_id=layer_idx)) for layer_idx in range(config.num_prefix_hybrid_layers, config.num_prefix_hybrid_layers + config.num_qwerky_layers())],
    ...
    def _forward_internal_embeddings(
        ...
        for i in range(qwerky_start, qwerky_end):
            ...
            if forward_chunk_count <= 1:
                x_hidden_state, last_layer_state = self._forward_qwerky_layer_hook(
                    layer, x_hidden_state, prv_stateList[qwerky_idx], 
                ...
                for j in range(forward_chunk_count):
                    ...
                    new_x_hidden_state, last_layer_state = self._forward_qwerky_layer_hook(
                        layer, 
    ...
    def _forward_qwerky_layer_hook(
        ...
        if self.checkpoint_function is not None:
            ...
            return self.checkpoint_function(
                layer, x_hidden_state, prv_stateList, position_embeddings
            )
        else:
            return layer(x_hidden_state, prv_stateList, position_embeddings)
