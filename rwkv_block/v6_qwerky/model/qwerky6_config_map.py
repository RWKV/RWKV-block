...
from ..block.qwerky6_block_config_map import Qwerky6BlockConfigMap
from transformers.models.qwen2.configuration_qwen2 import Qwen2Config

@dataclass
class Qwerky6ConfigMap(Qwerky6BlockConfigMap):
    ...
    max_position_embeddings: int = 32668
    ...
    @staticmethod
    def normalize(config_map: any) -> 'Qwerky6ConfigMap':
        '''
        Converts either maps, objs or configmaps
        '''
        if isinstance(config_map, Qwerky6ConfigMap):
            return config_map

        if isinstance(config_map, dict):
            return Qwerky6ConfigMap(**config_map)

        if hasattr(config_map, '__dict__'):
            return Qwerky6ConfigMap(**config_map.__dict__)
    ...
    @staticmethod
    def from_model_state_dict(state_dict: dict, **kwargs) -> 'Qwerky6ConfigMap':
        '''
